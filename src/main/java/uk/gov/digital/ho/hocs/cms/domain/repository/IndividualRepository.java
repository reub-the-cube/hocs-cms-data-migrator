...
public interface IndividualRepository extends CrudRepository<Individual, BigDecimal> {
    ...
    @Query(value = "SELECT * FROM individual WHERE caseid = ?1 AND type = ?2", nativeQuery = true)
    Individual findIndividualComplainantByCaseId(BigDecimal caseId, String type);
}
